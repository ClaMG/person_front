...
function TodosUsuarios() {
    ...
    const handleSubmit = async () => {
      ...
      try {
        ...
        if (response.ok) {//Verifica se a resposta foi bem-sucedida
          ...
          if (Array.isArray(result) && result.length > 0) {//Verifica se deu certo
              setMensagem(`Usuário encontrado: ${JSON.stringify(result, null, 2)}`);//Mostra os dados do usuário
          } else {
               setMensagem(`Nenhum usuario encontrado`);//mostra o erro
          }  
    ...
    return(//html do componente //form: chama a função de envio "handleSubmit"
      <div style={{ padding: '20px', maxWidth: '400px', margin: 'auto' }}>
        <h3>Aparecer Todos Os Usuário</h3>
